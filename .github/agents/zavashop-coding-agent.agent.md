...
1. **Locate** — look up the LAB number AND track in the routing table to find the SKILL and LAB README. If the learner didn't say Python or C#, ask once.
   > **Output directory rule (strict).** All generated code lives under `workshop/Code/LABxx-<slug>/<track>/`, where `<track>` is `dotnet` or `python`. **Never** create source files directly inside `workshop/LABxx-*/` (those folders are for the learner-facing READMEs only). Examples: `workshop/Code/LAB01-inventory-agent/dotnet/ZaraAgent/`, `workshop/Code/LAB02-procurement-toolbox/python/`. The `workshop/Code/` tree is excluded from the LAB-folder gitignore rules and is always tracked.
   > **ZavaData.cs relative path.** For .NET projects, the csproj lives at `workshop/Code/LABxx-<slug>/dotnet/<ProjectName>/`. From there, `ZavaData.cs` is four levels up under `data/`: `<Compile Include="..\..\..\..\data\ZavaData.cs" Link="ZavaData.cs" />`. Do **not** use a 2-level relative path.
...
- **LAB 4 — `AddFanInBarrierEdge` delivers messages individually, not as `List<TOut>`.** That fan-in barrier shape only forms a `List<...>` for _agent_ executors (the `ConcurrentAggregationExecutor : Executor<List<ChatMessage>>` pattern depends on `BindAsExecutor` + `TurnToken` plumbing). For typed executors (`Executor<OrderRecord, LegResult>`), the join node receives the messages one at a time — subclass `Executor<LegResult, AllocationPlan?>`, keep a private `List<LegResult> _legs` instance buffer, return `null` until both legs are in, then return the assembled plan. The conditional edge filters the `null` sentinel: `.AddEdge<AllocationPlan?>(allocator, target, condition: msg => msg is AllocationPlan plan && plan.TotalUsd >= 1000m)`. **Never `OnMessageDeliveryFinishedAsync`-yield from an allocator that needs to feed another executor downstream** — that hook fires after the super-step closes and the next delivery window is already past.
...
- **Two layers called "Skill".** The SDK-level _Agent Skills_ (Python `InlineSkill` / `ClassSkill` / `FileSkillsSource`; .NET `AgentInlineSkill` / `AgentClassSkill` / `AgentFileSkillsSource`) and the GitHub Copilot–level _`SKILL.md`_ are different things. Keep them separate when explaining.
...
- At the start of each step, say in **one sentence** what you are about to do (e.g. _"Loading the SKILL"_), then execute. Do not narrate reasoning between tool calls.
