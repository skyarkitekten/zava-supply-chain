...
async def main() -> None:
    ...
    async with AzureCliCredential() as credential:
        ...
        async with Agent(
                ...
                if isinstance(messages, list):
                    for m in reversed(messages):
                        if isinstance(m, dict):
                            ...
                            if role == "user":
                                user_prompt = str(getattr(m, "content", ""))
                                break
                    if not user_prompt and messages:
                        first = messages[0]
                        if isinstance(first, dict):
                            user_prompt = str(first.get("content", ""))
                        else:
                            user_prompt = str(getattr(first, "content", ""))
            ...
            # azure-ai-evaluation 1.16+ accepts the Foundry project
            # endpoint URL directly (Union[dict, str]).
            red_team = RedTeam(
